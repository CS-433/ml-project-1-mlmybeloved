...
def load_data(train=True, mode="ALL"):
    ...
    if train:
        ...
        res = np.genfromtxt(path_dataset, delimiter=",", skip_header=1, usecols=[0])
    return res, values
...
def replace_min_999_by_col_mean(x):
    ...
    for i in range(x.shape[1]):
        ...
        col_mean = np.ma.masked_array(col, mask_col).mean(
            axis=0
        )  # Compute mean without the -999
        x[:, i] = np.where(
            col == -999, col_mean, col
        )  # Replace -999 by mean or keep column
    return x
...
def ridge_regression(y, tx, lambda_):
    ...
    w = (
        np.linalg.inv(tx.T @ tx + 2 * np.size(y) * lambda_ * np.eye(tx.shape[1]))
        @ tx.T
        @ y
    )
    mse = compute_mse(y, tx, w)
...
def reg_logistic_regression(y, tx, lambda_, initial_w, max_iters, gamma):
    ...
    for n_iter in range(max_iters):
        gradient = (
            compute_log_gradient(y, tx, w) + lambda_ * w * 2
        )  # derivative of lambda/2 * w^2
        w = w - gamma * gradient
    loss = compute_log_loss(y, tx, w)
    return w, loss
...
def ridge_lambdas_and_threshold(y_tr, tx_tr, y_te, tx_te):
    ...
    print(
        "lambda=",
        recorded_vals[optindex][2],
        "best threshold=",
        recorded_vals[optindex][1],
        "accruacy=",
        recorded_vals[optindex][0],
    )
